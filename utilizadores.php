...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nome === '' || $email === '') {
        ...
        if ($uid > 0) {
            ...
            if ($senha_raw === '') {
                setFlash('error', 'Palavra-passe é obrigatória para novos utilizadores.');
            ...
            if ($chk->get_result()->num_rows > 0) {
                $chk->close();
                setFlash('error', 'Já existe um utilizador com este e-mail.');
                header('Location: utilizadores.php');
                exit();
            }
            $chk->close();
...
if (isset($_GET['desativar']) && is_numeric($_GET['desativar'])) {
    ...
    if ($id === (int)$_SESSION['usuario_id']) {
        ...
        $stmt->execute();
        $stmt->close();
        setFlash('success', 'Utilizador desativado.');
    }
    header('Location: utilizadores.php');
    exit();
}
...
if (isset($_GET['ativar']) && is_numeric($_GET['ativar'])) {
    ...
    $stmt->execute();
    $stmt->close();
    setFlash('success', 'Utilizador ativado.');
    header('Location: utilizadores.php');
    exit();
}
...
<div class="main-content">
    <div class="header d-flex justify-content-between align-items-center">
        <div>
            ...
            <small class="text-muted">Gerir utilizadores do sistema</small>
        </div>
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#userModal" onclick="clearUserForm()">
            <i class="bi bi-plus-circle me-2"></i>Novo Utilizador
        </button>
    </div>
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" name="perfil">
                            ...
                            <option value="utilizador" <?php echo ($f_perfil === 'utilizador') ? 'selected' : ''; ?>>Utilizador</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php if ($usuarios->num_rows === 0): ?>
                                <tr><td colspan="6" class="text-center text-muted py-4">Nenhum utilizador encontrado</td></tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            ...
                                            <?php if (!$u['ativo']): ?>
                                                <a href="utilizadores.php?ativar=<?php echo $u['id']; ?>" class="btn btn-sm btn-outline-success" title="Ativar" onclick="return confirm('Ativar este utilizador?')"><i class="bi bi-check"></i></a>
                                            <?php endif; ?>
                                            <?php if ((int)$u['id'] !== (int)$_SESSION['usuario_id']): ?>
                                                <?php if ($u['ativo']): ?>
                                                    <a href="utilizadores.php?desativar=<?php echo $u['id']; ?>" class="btn btn-sm btn-outline-danger" title="Desativar" onclick="return confirm('Desativar este utilizador?')"><i class="bi bi-x-lg"></i></a>
                                                <?php endif; ?>
                                            <?php endif; ?>
                                        </div>
                                    </td>
        ...
        <div class="modal-content">
            <form method="POST" action="utilizadores.php">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="userSubmitBtn">Criar Utilizador</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function clearUserForm() {
        document.getElementById("userModalLabel").textContent = "Novo Utilizador";
        document.getElementById("userSubmitBtn").textContent = "Criar Utilizador";
    ...
    function editUser(u) {
        document.getElementById("userModalLabel").textContent = "Editar Utilizador";
